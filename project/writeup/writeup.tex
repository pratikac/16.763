...
airline operations~\cite{barnhart1998flight}, even optimization problems from other fields such as graph partitioning for VLSI circuits \cite{vanderbeck1994thesis}, satisfiability problems \cite{hansen1998mixed}, graph coloring~\cite{mehrotra1996column} etc. are enormously large.
...
\aeq{
    ...
    \sum_{p \in P}\ \l_p &= 1,\label{eqn:rcsp_rlpm_convexity}\\
...
\While{nodes in tree}
{
    ...
    \item update lower bound $J_l, J^*$
    \item If solution $J_c$ is fractional,
        \begin{enumerate}[(a)]
        \item if $J_c \geq J_u$, discard node
        \item if $J_c < J_u$, create two new nodes (branch)
        \end{enumerate}
...
\ilist{
    ...
    \item $\a_{fr}$ is an indicator variable that is 1 if route $r$ contains flight $f$;
    ...
    \item $R^T$ is the number of routes that span an arbitrary time $T$, also known as the count-line;
    \item $N^T$ is the number of nodes with corresponding ground arcs $g_n^+$ that span the count-line;
...
\ilist{
    ...
    \item $\n_{cr} = \ind \rbrac{\trm{route}\ r\ \trm{contains short connect}\ c}$;
    \item $\eta_{cp} =\ind \rbrac{\trm{pairing}\ p\ \trm{contains short connect}\ c}$.
}
...
\ilist{
    \item $S$ is the set of feasible maintenance routing solutions;
...
\enum{
    \item Create initial crew pairings and maintenance solutions
        \begin{enumerate}[(a)]
            \item We can directly use ideas from Sec.~\ref{ssec:col_gen_ecp} to generate a set of MR solutions. In fact, if we solve the MR problem~\eqref{eqn:mr}, it generates one solution --- without adding any more MR solutions to this, we would obtain the usual sequential solution to the scheduling problem.
            \item For generating initial solutions to the CP problem, we use ideas from multi-label shortest path as described in Sec.~\ref{ssec:eg_shortest_path}.
        \end{enumerate}
    \item Solve current LP and compute dual variables
    \item Generate a new CP solution using reduced costs
        \begin{enumerate}[(a)]
            \item We use ideas from multi-label, resource constrained shortest path problems to do this (cf. Sec.~\ref{ssec:rcsp}). However, in the ECP model, we also have to incorporate the dual variables for the short connects, i.e., the reduced cost of a pairing becomes
            $$
...
\enum{
    ...
    $$
    and similarly for the edges that are set to 1, we include at least one path that has that edge,
    $$
    P^{''} = \trm{Sample}\left(\cbrac{p \in \trm{all paths}: x_{pij} =1, x_{ij}=1} \right).
    $$
    The modified set $P' \cup P^{''}$ is used for column generation. It has a path $p$ if some $x_{ij} \in X_{bnb}$ on that path is fixed to 1 and removes all paths who have at least one $x_{ij} = 0$ on them.
}
...
\aeqs{
    P_{init} = [0-2&-1-3-4-5],\ [0-1-3-5] \\
    P_1 &= P_{init} \cup [0-1-3-5]\\
    P_2 &= P_1 \cup [0-2-1-3-5]\\
    P_3 &= P_3 \cup [0-1-4-5]\\
